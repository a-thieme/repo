...
func TestConcurrentCommands_TwoProducers(t *testing.T) {
	...
	allExactlyRF := true
	hasUnderReplication := false
	hasOverReplication := false
	for target, count := range uniqueTargets {
		if count < cfg.replicationFactor {
			allExactlyRF = false
			hasUnderReplication = true
			t.Logf("  Command %s under-replicated: %d < %d", target, count, cfg.replicationFactor)
		} else if count > cfg.replicationFactor {
			allExactlyRF = false
			hasOverReplication = true
			t.Logf("  Command %s over-replicated: %d > %d", target, count, cfg.replicationFactor)
		} else {
			t.Logf("  Command %s exactly RF=%d", target, cfg.replicationFactor)
		}
	}

	if !allExactlyRF {
		if hasUnderReplication && hasOverReplication {
			t.Errorf("FAIL: Commands have both under- and over-replication (expected exactly RF=%d)", cfg.replicationFactor)
		} else if hasUnderReplication {
			t.Errorf("FAIL: Some commands under-replicated (expected exactly RF=%d)", cfg.replicationFactor)
		} else {
			t.Errorf("FAIL: Some commands over-replicated (expected exactly RF=%d)", cfg.replicationFactor)
		}
	} else {
		t.Logf("PASS: All %d concurrent commands replicated to exactly RF=%d", len(uniqueTargets), cfg.replicationFactor)
	}
}

func TestConcurrentCommands_ThreeProducers(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	cfg := integrationTestConfig{
		name:              "ConcurrentCommands3Producers",
		nodeCount:         5,
		replicationFactor: 3,
		cacheless:         false,
		runProducer:       false,
	...
	t.Logf("=== ConcurrentCommands3Producers RESULTS ===")
	t.Logf("Commands received: %d", commandsReceived)
	t.Logf("Nodes with claims: %d", totalClaims)
